...
# Encrypt a Secret
sops --encrypt secret.yaml > ./clusters/$AKS_NAME/demo_app_secret.yaml && rm secret.yaml

# Commit and Push secret
git add clusters/$AKS_NAME/demo_app_secret.yaml && git commit -m "Encrypted Secret" && git push
